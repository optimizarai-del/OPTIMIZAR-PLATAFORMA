      ...
      <div className="px-6 h-12 flex items-center gap-6">
        ...
        {hud && (
          <div className="hidden md:flex items-center gap-5 ml-4">
            ...
            <Stat label="Horas/semana" value={`${hud.horas_semana}h`} />
            {hud.requerimientos_nuevos > 0 && (
              <button onClick={() => nav('/requerimientos')}
                className="chip-accent animate-pulse cursor-pointer">
                <Zap size={10} /> {hud.requerimientos_nuevos} nuevo{hud.requerimientos_nuevos > 1 ? 's' : ''}
              </button>
            )}
          </div>
        )}
...
function Stat({ label, value, accent }) {
  return (
    <div className="flex flex-col items-center">
      <span className={`font-semibold text-sm tracking-tight ${accent ? 'text-accent' : 'text-primary'}`}>
        {value}
      </span>
